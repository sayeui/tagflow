...
- [x] **卡片间距修复**：消除虚拟滚动导致的上下行重叠/裁切，行高与 `item-size` 严格一致。
- [x] **无限滚动加载**：滚到底部按 page 追加拉取，直到 `items.length >= total`；与标签筛选/搜索联动重置分页。
- [x] **文件名搜索**：后端 `FileQuery` 增加关键词字段（`filename LIKE`，不区分大小写，中文友好），与标签筛选 AND 组合；前端 header 增搜索框，防抖触发。
- [x] **卡片/列表视图切换**：新增列表视图（每行一个文件，展示名/大小/时间/路径等），header 切换 toggle，视图偏好持久化（localStorage）。

## Acceptance Criteria（evolving）

- [x] 卡片视图：任意滚动位置无上下行重叠/裁切，缩略图、文件名、大小完整可见。
- [x] 文件数 > 50 时，向下滚动能持续加载后续文件直到全部呈现。
- [x] 输入文件名关键词后，列表仅显示文件名包含该关键词的文件（与已选标签 AND）。
- [x] 切换"列表/卡片"后，当前文件集合保持不变，偏好刷新后保留。
- [x] 搜索/标签切换/清空时，分页状态正确重置，不出现重复或遗漏。

## E2E 验证结果（2026-06-16）

**真实进程 e2e（76 文件库，后端 8080 + 前端 5173 + Chrome DevTools）：**

API 层（10 场景全绿）：分页 p1=50/p2=26/p3=0（total 76）；搜索 report=40 / image=30 / 中文(设计文档)=1；`%`_` 转义边界 50percent=1、alpha_beta=1（当字面量）；搜索+分页组合 report p2=0。

UI 层（5 验收点全绿）：
1. 卡片无重叠——DOM 测量相邻行 translateY 间距恒 176px、行高恒 176px、overflow=0。
2. 无限滚动——滚动触底 50/76 → 76/76。
3. 文件名搜索——输入 report 后 40/40，卡片全为 report-XX。
4. 视图切换——列表视图渲染（无缩略图，名称/大小/时间），集合保持 40/40。
5. 偏好持久化——`tagflow.viewMode=list` 写入 localStorage，刷新后自动恢复列表视图。

测试数据已清理：73 测试 png 删除、db(files/tags/tasks/file_tags) 归零、cache 缩略图清空。
